...
-- Q.3---- Total Revenue from Coffee Sales
...
WHERE 
     YEAR(sale_date)=2023 and (month(sale_date) between 9 and 12)
...
SELECT 
	ci.city_name,
	COUNT(DISTINCT c.customer_id) as unique_count
FROM city as ci
LEFT JOIN
customers as c
...
-- Q7 -- Top Selling Products by City
...
SELECT 
	...
	ROUND(
		cr.estimated_rent/ct.total_cx , 2) as avg_rent_per_count
FROM city_rent as cr
JOIN city_table as ct
ON cr.city_name = ct.city_name
ORDER BY ct.avg_sale_per_cust DESC;
...
(
	...
	ORDER BY ci.city_name,EXTRACT(MONTH FROM sale_date),EXTRACT(YEAR FROM sale_date)
),
growth_ratio
AS
(
...
SELECT 
	...
	ct.avg_sale_pr_cust,
	ROUND(
		cr.estimated_rent::numeric/ct.total_cust::numeric
